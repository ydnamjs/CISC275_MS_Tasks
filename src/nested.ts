...
export function toCSV(questions: Question[]): string {
    const csv = questions.reduce(
        (currentCSV: string, question: Question) =>
            currentCSV +
            "\n" +
            question.id +
            "," +
...
export function publishAll(questions: Question[]): Question[] {
    const quesionsPublished = questions.map(
...
export function sameType(questions: Question[]): boolean {
    const areMultipleChoice = questions.filter(
        (question: Question): boolean =>
            question.type.toLowerCase() === "multiple_choice_question"
    );
    return (
        areMultipleChoice.length === questions.length ||
        areMultipleChoice.length == 0
    );
}
...
export function duplicateQuestionInArray(
    ...
    const duplicatedQuestion = duplicateQuestion(newId, questions[ind]);
    const temp = [...questions];
    temp.splice(ind + 1, 0, duplicatedQuestion);
    return temp;
}
